...
public sealed class IngestController : ControllerBase
{
    ...
    public async Task<IActionResult> Put(string jobId, string name)
    {
        ...
        {
            _logger.LogDebug("jfc: ingest rejected (bad token/job) job={JobId} name={Name}", jobId, name);
            return Fail(StatusCodes.Status403Forbidden);
        }

        if (!IngestNames.IsValid(grant.FilePrefix, name))
        {
            _logger.LogDebug("jfc: ingest rejected (bad filename) job={JobId} name={Name} prefix={Prefix}", jobId, name, grant.FilePrefix);
        ...
        {
            _logger.LogWarning(ex, "jfc: ingest write failed job={JobId} name={Name}", jobId, name);
            TryDelete(partPath);
            return Fail(StatusCodes.Status404NotFound);
        }
        ...
        _logger.LogDebug(
            "jfc: ingest wrote job={JobId} name={Name} bytes={Bytes} elapsedMs={ElapsedMs}",
            jobId,
            name,
            bytes,
        ...
        {
            _logger.LogInformation("jfc: first playlist upload for job {JobId}", jobId);
        }
