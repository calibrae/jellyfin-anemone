...
public sealed class JobRouter : IJobRouter
{
    ...
    public RoutePlan? TryPlan(StreamState state, string outputPath, string commandLineArguments, TranscodingJobType jobType)
    {
        ...
        {
            _logger.LogDebug(ex, "jfc: not routing {Path}: exception while planning", outputPath);
            return null;
        }
    }
    ...
    private RoutePlan? TryPlanCore(StreamState state, string outputPath, string commandLineArguments)
    {
        ...
        {
            _logger.LogDebug("jfc: not routing {Path}: {Reason}", outputPath, analysis.NotRoutableReason);
            return null;
        }

        if (state.MediaSource?.Protocol != MediaProtocol.File)
        {
            _logger.LogDebug("jfc: not routing {Path}: media source protocol is {Protocol}, not File", outputPath, state.MediaSource?.Protocol);
            return null;
        }
        ...
        {
            _logger.LogDebug(
                "jfc: not routing {Path}: no agent satisfies requirements (hwaccels=[{Hwaccels}], encoders=[{Encoders}], decoders=[{Decoders}], filters=[{Filters}])",
                outputPath,
