// jfc: fork of MediaBrowser.MediaEncoding/Transcoding/TranscodeManager.cs @ Jellyfin v10.11.0.
// Kept as close to verbatim as possible; every deviation is marked with a "// jfc:" comment so this
...
public sealed class ClusterTranscodeManager : ITranscodeManager, IDisposable
{
    private readonly ILoggerFactory _loggerFactory;
    private readonly ILogger<ClusterTranscodeManager> _logger;
    ...
    // jfc: added dependencies — job placement + ingest token lifecycle.
    ...
    // jfc: AsyncKeyedLocker<string> (AsyncKeyedLock package) replaced with an in-file KeyedLock so we
    ...
    // jfc: jobs currently running on an agent, keyed by TranscodingJob.Id (== RemoteJobSpec.Id ==
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ClusterTranscodeManager"/> class.
    /// </summary>
    ...
    /// <param name="router">jfc: decides local-vs-remote and rewrites the command line.</param>
    /// <param name="tokenStore">jfc: mints/validates/revokes ingest bearer tokens.</param>
    public ClusterTranscodeManager(
        ...
        IIngestTokenStore tokenStore)
    {
        ...
        _logger = loggerFactory.CreateLogger<ClusterTranscodeManager>();
...
#pragma warning restore CA1849
        ...
        // jfc: a remote job has no local Process — Stop() would NRE on process!.StandardInput if the job
    ...
    // jfc: mirrors TranscodingJob.Stop()'s "q then wait 5s then kill" contract, over the agent's
    // control channel instead of local stdin/Process.Kill().
    private async Task StopRemoteJobAsync(IRemoteJob remoteJob)
    {
        try
        {
            _logger.LogInformation("jfc: stopping remote job {Id} with q command", remoteJob.Id);
            await remoteJob.SendStdinAsync("q\n").ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "jfc: error sending quit to remote job {Id}", remoteJob.Id);
        }

        var finished = await Task.WhenAny(remoteJob.Completion, Task.Delay(5000)).ConfigureAwait(false) == remoteJob.Completion;
        if (!finished)
        {
            _logger.LogInformation("jfc: killing remote job {Id} (did not exit within 5s of quit)", remoteJob.Id);
            try
            {
            ...
            {
                _logger.LogDebug(ex, "jfc: error killing remote job {Id}", remoteJob.Id);
            }
        }
    }
        ...
        string? workingDirectory = null)
    {
        ...
        // jfc: try to place the job on an agent before falling back to the upstream local-process path.
        ...
        {
            _logger.LogInformation("jfc: dry-run — would route {Path} to agent {Agent} ({Reason})", outputPath, plan.Agent.Info.Name, plan.Reason);
    ...
    // jfc: everything below in this region is new — the remote-job mirror of the local flow above.
        ...
        CancellationTokenSource cancellationTokenSource)
    {
        ...
        _logger.LogInformation("jfc: routing {Path} to agent {Agent}: {Reason}", outputPath, plan.Agent.Info.Name, plan.Reason);
        ...
        var commandLineLogMessageBytes = Encoding.UTF8.GetBytes(
            ...
            + Environment.NewLine
            + Environment.NewLine
            + $"jfc: routed to agent {plan.Agent.Info.Name}: {argvLine}"
            + Environment.NewLine
            + Environment.NewLine);
        ...
        var sink = new RemoteJobSink(
            pipe.Writer,
            pid => _logger.LogDebug("jfc: remote job {Id} started, agent pid {Pid}", jobId, pid),
            (exitCode, error) =>
            {
                ...
                {
                    _logger.LogWarning("jfc: remote job {Id} reported error: {Error}", jobId, error);
                }
        ...
        {
            _logger.LogWarning(ex, "jfc: failed to start job on agent {Agent}, falling back to local", plan.Agent.Info.Name);
        ...
        var ffmpegTargetFile = state.WaitForPath ?? outputPath;
        _logger.LogDebug("jfc: waiting for the creation of {0} (remote)", ffmpegTargetFile);
        while (!File.Exists(ffmpegTargetFile) && !transcodingJob.HasExited)
        {
            await Task.Delay(100, cancellationTokenSource.Token).ConfigureAwait(false);
        }
        ...
        if (transcodingJob.HasExited && !File.Exists(ffmpegTargetFile))
        {
            ...
            {
                _logger.LogError("jfc: remote job on {Agent} exited before producing output, and its teardown already ran — cannot fall back safely", plan.Agent.Info.Name);
            ...
            _logger.LogWarning("jfc: remote job on {Agent} exited before producing output, falling back to local", plan.Agent.Info.Name);
            RemoveFromActiveJobs(transcodingJob);
            _tokenStore.Revoke(jobId);
            return null;
        }

        _logger.LogDebug("jfc: file {0} created or transcoding has finished (remote)", ffmpegTargetFile);

        if (!transcodingJob.HasExited)
        {
            // jfc: no throttler for remote jobs — TranscodingThrottler dereferences job.Process! to write
            ...
            _logger.LogDebug("jfc: throttling not applied to remote job {Id}", jobId);
        ...
        _logger.LogDebug("jfc: TryStartRemoteAsync() finished successfully");
    ...
    // jfc: NOTE - deliberately does NOT dispose transcodingJob.CancellationTokenSource (nor call
    ...
    private void CleanUpFailedRemoteStart(TranscodingJob transcodingJob, Stream logStream, RoutePlan plan)
    {
        ...
        {
            _logger.LogDebug(ex, "jfc: error closing log stream after failed remote start");
        }
    }

    // jfc: remote-job equivalent of OnFfMpegProcessExited — same FinishJob bookkeeping, plus removing
    ...
    private void FinishRemoteJob(TranscodingJob job, StreamState state)
    {
        ...
        {
            _logger.LogDebug("jfc: remote job {Id} exit already handled by the local-fallback path, skipping FinishJob", job.Id);
            return;
        }
    ...
    // jfc: process parameter widened to Process? — remote jobs have no local Process at all.
    ...
    // jfc: split into FinishJob (shared by local and remote) + this thin local-only wrapper, so both
    ...
    // jfc: factored out of upstream's OnFfMpegProcessExited so remote jobs (FinishRemoteJob) get the
