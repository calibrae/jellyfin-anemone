...
/// <summary>The control-channel WebSocket endpoint a jfc-agent opens: <c>GET /Cluster/agents/ws</c>.</summary>
[ApiController]
[Route("Cluster")]
[AllowAnonymous]
public sealed class AgentWebSocketController : ControllerBase
{
    ...
    public async Task<IActionResult> AgentWebSocket()
    {
        ...
        {
            _logger.LogWarning("jfc: rejecting agent connection from {Remote}: SharedSecret not configured", HttpContext.Connection.RemoteIpAddress);
        ...
        {
            _logger.LogWarning("jfc: rejecting agent connection from {Remote}: bad or missing bearer token", HttpContext.Connection.RemoteIpAddress);
            return Unauthorized();
        }
