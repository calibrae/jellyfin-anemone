...
public sealed class ClusterHostedService : IHostedService, IDisposable
{
    ...
    public ClusterHostedService(AgentHub hub, ILogger<ClusterHostedService> logger)
    {
        _hub = hub;
        _logger = logger;
    }
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation(
            "jfc: Cluster plugin {Version} loaded — enabled={Enabled} dryRun={DryRun} ingestBase={IngestBase} sharedSecret={SecretState}",
            pluginVersion,
        ...
        {
            _logger.LogWarning(
                "jfc: SharedSecret is not configured — no agent will be able to authenticate. " +
                "Set it on the Cluster plugin config page.");
        }
