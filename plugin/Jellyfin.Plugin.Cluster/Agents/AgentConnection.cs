...
/// <summary>
/// Wraps one accepted <see cref="WebSocket"/> to a connected jfc-agent, past the hello/welcome handshake
...
public sealed class AgentConnection : IAgentConnection
{
    ...
    internal async Task RunAsync(CancellationToken hostCancellationToken)
    {
        ...
        finally
        {
            ...
            {
                _logger.LogDebug(ex, "jfc: agent {Name} writer loop faulted", _info.Name);
            }
            ...
            {
                _logger.LogDebug(ex, "jfc: agent {Name} ping loop faulted", _info.Name);
            }
    ...
    internal async Task CloseAsync(string reason, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug(ex, "jfc: error closing agent {Name} connection ({Reason})", _info.Name, reason);
        }
    }
    ...
    public async Task<IRemoteJob> StartJobAsync(RemoteJobSpec spec, IRemoteJobSink sink, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException($"jfc: duplicate job id {spec.Id} on agent {_info.Name}");
        }
        ...
        catch (OperationCanceledException) when (timeoutCts.IsCancellationRequested)
        {
            ...
            throw new TimeoutException(
                $"jfc: agent {_info.Name} did not confirm start of job {spec.Id} within {timeoutSeconds}s");
        }
    ...
    private void TryEnqueueKill(string id)
    {
        ...
        {
            _logger.LogDebug(ex, "jfc: failed to send best-effort kill for job {Id} on agent {Name}", id, _info.Name);
        }
    }
    ...
    private async Task WriterLoopAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug(ex, "jfc: agent {Name} writer loop ended", _info.Name);
        }
    }
    ...
    private async Task ReadLoopAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogWarning(ex, "jfc: agent {Name} sent malformed frame: {Json}", _info.Name, json);
                continue;
            }
    ...
    private void Dispatch(Frame frame)
    {
        switch (frame)
        {
            ...
            case UnknownFrame unknown:
                _logger.LogDebug("jfc: agent {Name} sent unknown frame type '{Type}'", _info.Name, unknown.Type);
                break;
            default:
                _logger.LogDebug("jfc: agent {Name} sent unexpected frame {Type}", _info.Name, frame.GetType().Name);
                break;
        }
    }
    ...
    private void HandleStarted(StartedFrame started)
    {
        ...
        {
            _logger.LogDebug("jfc: agent {Name} 'started' for unknown job {Id}", _info.Name, started.Id);
        }
    }
    ...
    private void HandleExit(ExitFrame exit)
    {
        ...
        {
            _logger.LogDebug("jfc: agent {Name} 'exit' for unknown job {Id}", _info.Name, exit.Id);
            return;
        }
        ...
        if (!entry.StartAck.Task.IsCompleted)
        {
            var message = string.IsNullOrEmpty(exit.Error)
                ? $"jfc: job {exit.Id} exited (code {exit.Code}) before agent confirmed start"
                : $"jfc: job {exit.Id} exited (code {exit.Code}) before agent confirmed start: {exit.Error}";
    ...
    private void HandleError(ErrorFrame error)
    {
        ...
        {
            _logger.LogWarning("jfc: agent {Name} job {Id} error: {Message}", _info.Name, error.Id, error.Message);
        }
        else
        {
            _logger.LogWarning("jfc: agent {Name} error: {Message}", _info.Name, error.Message);
        }
    }
    ...
    private void FailAllPendingJobs()
    {
        foreach (var id in _pendingJobs.Keys.ToList())
        {
            ...
            entry.StartAck.TrySetException(
                new InvalidOperationException($"jfc: agent {_info.Name} connection lost while starting job {id}"));
        }
    }
