...
public class RoutePlannerTests
{
    ...
    public void Rewrite_TranscodeFixture_MatchesIndependentlyComputedExpectation()
    {
        ...
        expected[segValueIndex] = $"{ingestBase}/Cluster/ingest/{jobId}/{Fixtures.Md5}%d.ts";
        expected[^1] = $"{ingestBase}/Cluster/ingest/{jobId}/{Fixtures.Md5}.m3u8";
    ...
    public void Rewrite_TranscodeFixture_InsertsSixTokensImmediatelyBeforeLastElement()
    {
        ...
        Assert.Equal($"http://10.240.0.1:8096/Cluster/ingest/job1/{Fixtures.Md5}.m3u8", rewritten[^1]);
    }
