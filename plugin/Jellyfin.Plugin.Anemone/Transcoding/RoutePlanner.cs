...
/// <summary>
/// anemone: pure ffmpeg-argv analysis and rewriting, split out of <see cref="JobRouter"/> so it's testable
...
public static class RoutePlanner
{
    ...
    public static RouteAnalysis Analyze(IReadOnlyList<string> argv)
    {
        ...
        // anemone: requirements are conceptually a set (e.g. the real HLS transcode command line passes
    ...
    public static IReadOnlyList<string> Rewrite(IReadOnlyList<string> argv, string ingestBase, string jobId, string token)
    {
        ...
        for (var i = 0; i < result.Count - 1; i++)
        {
            if (result[i] == "-hls_segment_filename")
            {
                ...
                result[i + 1] = $"{baseUrl}/Anemone/ingest/{jobId}/{basename}";
                break;
            }
        }
        ...
        result[outputIndex] = $"{baseUrl}/Anemone/ingest/{jobId}/{playlistBasename}";
