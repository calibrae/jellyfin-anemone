//! `jfc-mock-server`: a fake Jellyfin Cluster plugin so `jfc-agent` can be exercised end-to-end
//! without a real Jellyfin. Speaks the control WebSocket (`/Cluster/agents/ws`) and the ingest
//! PUT endpoint (`/Cluster/ingest/{job}/{name}`) per `PROTOCOL.md`.
...
use jfc_agent::protocol::{
    peek_frame_type, validate_ingest_filename, AgentMessage, ServerInfo, ServerMessage,
};

#[derive(Parser, Debug)]
#[command(
    name = "jfc-mock-server",
    about = "Fake Jellyfin Cluster plugin for testing jfc-agent"
)]
struct Args {
...
async fn main() -> Result<()> {
    ...
    println!("jfc-mock-server listening on http://{local_addr}");
    println!("  control:  ws://{local_addr}/Cluster/agents/ws");
    println!(
        "  ingest:   {}/Cluster/ingest/<job>/<name>",
        state.ingest_base
    );
    ...
    let app = Router::new()
        .route("/Cluster/agents/ws", get(ws_upgrade_handler))
        .route("/Cluster/ingest/{job}/{name}", put(ingest_handler))
...
fn testsrc_argv(ingest_base: &str, id: &str, token: &str) -> Vec<String> {
    let seg = format!("{ingest_base}/Cluster/ingest/{id}/testjob%d.ts");
    let playlist = format!("{ingest_base}/Cluster/ingest/{id}/testjob.m3u8");
