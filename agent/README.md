...
Rust half of `jellyfincluster`: `jfc-agent` is a daemon that connects to a Jellyfin cluster
plugin over WebSocket and runs `ffmpeg` transcodes on request, pushing HLS output straight back
to the plugin over HTTP PUT. `jfc-mock-server` is a fake plugin for exercising the agent without
...
Produces `target/release/jfc-agent` and `target/release/jfc-mock-server`.

## Running the agent

```sh
jfc-agent --config /etc/jfc-agent.toml
```
...
```sh
jfc-agent \
  --server-url ws://10.240.0.1:8096/Cluster/agents/ws \
  --secret "$(vault read -field=value secret/data/infra/jfc)" \
  --name trish \
...
### Config reference (`jfc-agent.example.toml`)

| key | required | default | notes |
|---|---|---|---|
| `server_url` | yes (here or `--server-url`) | -- | e.g. `ws://10.240.0.1:8096/Cluster/agents/ws` |
...
mkdir -p /tmp/jfc-out
./target/release/jfc-mock-server \
...
./target/release/jfc-agent \
  --server-url ws://127.0.0.1:8097/Cluster/agents/ws \
  --secret devsecret \
  --name dev-agent \
...
`stderr`/`exit` frame it receives. Segments and the playlist land in `/tmp/jfc-out` as
...
```sh
jfc-mock-server --listen 127.0.0.1:0 --secret devsecret --out-dir /tmp/jfc-out --once
```
...
replace the segment/playlist output paths with `{ingest}/Cluster/ingest/{id}/<prefix>...` plus a
...
[
  ...
  "-hls_segment_filename", "{ingest}/Cluster/ingest/{id}/myjob%d.ts",
  "-method", "PUT", "-http_persistent", "1",
  "-headers", "Authorization: Bearer {token}\r\n",
  "{ingest}/Cluster/ingest/{id}/myjob.m3u8"
]
```

```sh
jfc-mock-server --secret devsecret --out-dir /tmp/jfc-out --job-file job.json
```
...
- `tests/e2e.rs`: spawns the real `jfc-mock-server` and `jfc-agent` binaries against a real local
...
Then edit `/etc/jfc-agent.toml` (it's installed with the placeholder secret from
`jfc-agent.example.toml`) and:

```sh
sudo launchctl kickstart -k system/net.calii.jfc-agent
```

Logs: `/var/log/jfc-agent/jfc-agent.log` (stdout) and `jfc-agent.err.log` (stderr/tracing).

```sh
sudo ./uninstall.sh       # stop + remove daemon + binary, leaves /etc/jfc-agent.toml
sudo ./uninstall.sh -c    # also remove /etc/jfc-agent.toml
```
...
agent/
  src/
    main.rs      -- jfc-agent entry point: config, probe, signal handling, wires ws.rs + job.rs
    ...
    bin/
      jfc-mock-server.rs -- fake plugin: control WS + ingest PUT + interactive/--once test driver
  tests/
    e2e.rs         -- full-binary integration test
    fixtures/      -- captured ffmpeg -version/-hwaccels/-encoders/-decoders/-filters output
  launchd/
    net.calii.jfc-agent.plist
  install.sh / uninstall.sh
  jfc-agent.example.toml
```
